...
def calculate_rloo_baseline(prompts, reward, mask, normalize_variance):
    '''
    ...
    baseline_std = torch.ones_like(reward)
    ...
    for i in range(len(unique_prompts)):
        ...
        if mask[prompt_idx].sum() <= 1:
            ...
            if normalize_variance:
                if mask[prompt_idx].sum() <= 2:
                    continue
                n = mask[prompt_idx].sum()
                loo_var = torch.matmul(rloo_mat, (reward[prompt_idx] * mask[prompt_idx] - rloo) ** 2) / (mask[prompt_idx].sum() - 2)
                loo_std = torch.sqrt(loo_var)
                baseline_std[prompt_idx] = loo_std + 1e-3
